...
class Socr(models.Model):
    socr = models.CharField(
        max_length=10, primary_key=True)   # duplicated values allowed!
...
class Kladr(models.Model):
    ...
    def get_vilage_code(self):
        return self.code[:11]

    def __str__(self):
        return "region: %s, district: %s, city: %s" % (self.code[:2], self.code[2:5], self.code[5:8])
#---


class Street(models.Model):
    code = models.CharField(
        max_length=17, primary_key=True)   # SS RRR GGG PPP UUUU AA
    # level == 5
    name = models.CharField(max_length=40)
    ...
    def get_city_code(self):
        return "%s%s" % (self.code[:8], '0' * 5)

    def get_vilage_code(self):
        return "%s%s" % (self.code[:11], '0' * 2)

    def get_street_code(self):
        return self.code[:17]

    def __unicode__(self):
        return "region: %s, district: %s, city: %s, street: %s" % (self.code[:2], self.code[2:5], self.code[5:8], self.name)

    def get_full_address(self):
        ''' returns full address of the street '''
        res = []
        res.append(Kladr.objects.get(
            code__startswith=self.get_region_code(), level=REGION_LEVEL).name)

        try:
            # append city name if exists
            res.append(Kladr.objects.get(
                code__startswith=self.get_city_code(), level=CITY_LEVEL).name)
        except Kladr.DoesNotExist:
            pass

        try:
            # append vilage name if exists
            res.append(Kladr.objects.get(code__startswith=self.get_vilage_code(
            ), level=VILAGE_LEVEL).name)
        except Kladr.DoesNotExist:
            pass

        res.append("%s %s" % (self.name, self.socr))
        return ",".join(res)


class Doma(models.Model):
    code = models.CharField(
        max_length=19, primary_key=True)   # SS RRR GGG PPP UUUU AA
    # level == 6
    name = models.CharField(max_length=40)
    ...
    def get_full_address(self):
        ''' returns full address of the house '''
        res = []
        res.append(Kladr.objects.get(
            code__startswith=self.get_region_code(), level=REGION_LEVEL).name)
        ...
        try:
            # append street name if exists. Exactly. There may be house without
            # street.
            street = Street.objects.get(
                code__startswith=self.get_street_code())
            res.append("%s %s" % (street.name, street.socr))
        except Kladr.DoesNotExist:
            pass
...
def by_code(code, level):
    ...
    curs = connection.cursor()  # @UndefinedVariable
    curs.execute(
        "select code,name,socr from " + sql + " order by code limit 1",
